...
namespace AlloeHRMSystem.UI.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        {
            var employeesbyhr = _employeeRepository.HrSearchAllEmployee(searchString);

            if (!String.IsNullOrEmpty(searchString) && !employeesbyhr.Any())
            {
        ...
        [HttpPost]
        public IActionResult HrCreateEmployee(CreateEmployeeViewModel employee)
        {
            if (ModelState.IsValid)
            {
                string uniueFileName = ProcessUploadedFile(employee);
                Employee newEmployee = new Employee
                {
                    ...
                    LegalDayOff = employee.LegalDayOff,
                    PhotoPath = uniueFileName

                };

                _employeeRepository.HrAddEmployee(newEmployee);
                var successMessage = "Employee Successfully Created. Name: " + newEmployee.EmployeeId;
                TempData["successAlert"] = successMessage;
               // return View(successMessage, "_PartialMessage");
                return PartialView("_PartialMessage");
              //  return RedirectToAction("HrGetAllEmployee", new { id = newEmployee.EmployeeId });
            }
            return View();
        }
        ...
        [HttpPost]
        public IActionResult HrEditEmployee(EmployeeEditViewModel model)
        {
            if (ModelState.IsValid)
            {
        ...
        {
            var employeesbyma = _employeeRepository.MaQueryAllEmployee(searchString);

            if (!String.IsNullOrEmpty(searchString) && !employeesbyma.Any())
            {
                employeesbyma = employeesbyma.Where(t => t.Firstname.ToLower().Contains(searchString)
                    || t.Lastname.ToLower().Contains(searchString)
                    || t.JobTitle.ToLower().Contains(searchString)
                    || t.Email.ToLower().Contains(searchString)
